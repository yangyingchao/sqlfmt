...
const KW: &[&str] = &[
    "SELECT", "FROM", "WHERE", "INTO", "VALUES",
    "SET", "ORDER", "BY", "GROUP", "HAVING", "LIMIT", "OFFSET",
    "JOIN", "LEFT", "RIGHT", "INNER", "OUTER", "CROSS", "FULL",
    "NATURAL", "USING", "WHEN", "THEN", "ELSE", "END",
    "UNION", "INTERSECT", "EXCEPT", "RETURNING", "CASE",
    "INSERT", "UPDATE", "DELETE", "CREATE", "DROP", "ALTER",
    "TABLE", "DISTRIBUTED", "PARTITION",
];
...
pub fn apply_width(sql: &str, cfg: &FormatterConfig) -> String {
    ...
    for line in sql.lines() {
        let t = line.trim();
        if t.is_empty() || t.starts_with("--") { r.push_str(line); r.push('\n'); continue; }
        if t.len() <= cfg.print_width { r.push_str(line); r.push('\n'); continue; }
...
fn fmt(sql: &str, margin: usize, indent: &str, tw: usize, lvl: usize) -> String {
    if lvl > 10 {
        ...
        return if lvl == 0 { sql.to_string() } else { format!("{}{}", ind, sql) };
    }
    ...
    if lvl == 0 {
        if let Some(s) = out.strip_prefix(indent) { return s.to_string(); }
    }
    out
}
...
fn find_breaks(sql: &str) -> Vec<(usize, u8)> {
    ...
    while i < n {
        let ch = chars[i];
        if ins { if ch == sc { ins = false; } i += 1; continue; }
        if ch == '\'' || ch == '"' { ins = true; sc = ch; i += 1; continue; }
        if ch == '(' { depth += 1; i += 1; continue; }
        if ch == ')' { depth -= 1; i += 1; continue; }
        if ch == ',' && depth == 0 { bs.push((i + 1, 1)); i += 1; continue; }
        if ch.is_whitespace() && depth == 0 && i > 0 {
            let mut ws = i;
            while ws > 0 && !chars[ws - 1].is_whitespace()
                && chars[ws - 1] != '(' && chars[ws - 1] != ')'
                && chars[ws - 1] != ',' {
                ws -= 1;
            }
            if ws < i {
                ...
                if is_kw(&u) {
                    if u == "BY" {
                        ...
                        if let Some(ul) = last.split_whitespace().last() {
                            ...
                            if ul == "ORDER" || ul == "GROUP" || ul == "DISTRIBUTED" || ul == "PARTITION" {
                                i += 1; continue;
                            }
                        }
                    }
...
fn reemit(out: &mut String, text: &str, margin: usize, indent: &str,
          tw: usize, lvl: usize) {
    let used = lvl * if indent == "\t" { tw } else { indent.len() };
    let avail = margin.saturating_sub(used);
    if text.len() <= avail {
...
fn is_kw(s: &str) -> bool { KW.contains(&s) }
